...
- npm 脚本可以做很多事情：构建客户端包、执行测试、生成文档等。它基本上就是一个微型脚本调用工具，所以只要你喜欢，放什么都行。

### 一.什么是 npm 脚本

- npm 允许在 package.json 文件，使用 scripts 字段定义脚本命令：scripts 字段是一个对象。它的每一个属性，对应一段脚本。

```json
//比如，build 命令对应的脚本是 node build.js。
"scripts":{
    "build":"node build.js"
}
```
...
内置的 npm 脚本都有简写形式，用户自定义的脚本需要完整执行`npm run <user defined>`：
...
首先，通过 `npm_package`前缀，npm 脚本可以拿到 package.json 里面的字段。比如，下面是一个 package.json:
...
[官方 api](https://docs.npmjs.com/cli/v8/using-npm/scripts#npm-restart)
