...
其实，我们在日常工作中，经常接触一个和它很像的东西，那就是 git commit。在创建 commit 时，git 会对整个项目的所有文件做一个快照，这里快照记录的并不是文件的内容，而是文件的索引。

举个例子：假设一个项目中有 A、B 两个文件，其中 A 文件被修改了，而 B 文件保持不变。我们将修改后的新的 A 文件的索引记为 A'。在变化发生后，A 和 A' 是共存的，变化前的那一次快照指向 A，变化后的这一次快照指向 A'。
而未被修改到的 B 文件，将会原封不动地呆在原地，被新版本的快照所复用，如下图所示：
<img src="./imgs/git记录变更.png">
